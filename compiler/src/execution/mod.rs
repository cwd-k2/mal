...
pub(crate) use packed::{PackedDataPlan, StablePackedAccess};
...
pub(crate) struct Program {
    ...
    pub(crate) packed_data: PackedDataPlan,
...
pub(crate) fn lower(lowered: closure_ast::Program, enabled: OptimizationSet) -> Program {
    ...
    let packed_data = PackedDataPlan::new(&lowered, &control, &applications);
    debug_assert!(packed_data.is_valid(&lowered, &control, &applications));
