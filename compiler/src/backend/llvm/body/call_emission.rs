...
impl FunctionEmitter<'_> {
    pub(super) fn emit_call(
        ...
        let arguments = if target.parameter.ty == Type::Unit {
            ...
            let argument = self.adapt_buffer_argument(
                argument,
                self.optimizations.uses_direct_buffer(target.id),
            )?;
    ...
    fn adapt_buffer_argument(
        &mut self,
        value: EmittedValue,
        target_uses_direct_buffer: bool,
    ) -> Option<EmittedValue> {
        if !super::super::optimization::type_contains_buffer(&value.ty) {
            return Some(value);
        }
        ...
        match ty {
            ...
            Type::Product(elements) => {
                ...
                for (index, element) in elements.iter().enumerate() {
                    if !super::super::optimization::type_contains_buffer(element) {
                        continue;
                    }
            ...
            _ => (!super::super::optimization::type_contains_buffer(ty)).then(|| value.to_string()),
        }
    }
