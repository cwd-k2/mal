use malc::diagnostic::Diagnostic;
...
fn exercise(id: u32, text: String) {
    ...
    match malc::formatter::format(&source) {
        Ok(formatted) => {
            let formatted_source = source_file(id, formatted.clone());
            assert_eq!(
                malc::formatter::format(&formatted_source).expect("reformat accepted source"),
                formatted
            );
        }
        Err(diagnostic) => assert_valid_diagnostic(&source, &diagnostic),
    }
    if let Err(diagnostic) = malc::pipeline::check(&source) {
