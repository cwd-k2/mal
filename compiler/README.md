...
The compiler implements the pipeline from source loading through native C compilation and linking. The implemented language slice includes closures, products, sums, fixed-width integers, strict floating point, byte and Engram literals, Engram observation and concatenation, numeric conversions, target storage-size expressions, untyped `Ptr` memory access for numeric scalars, pointers, and Engram descriptors, external opaque types, and the aggregate C host ABI. See the [compiler responsibilities](../docs/implementation/responsibilities.md) and [implementation notes](../docs/implementation/compiler.md) for the current structure.
