...
The compiler implements the pipeline from source loading through native C compilation and linking. The implemented language slice includes closures, products, sums, fixed-width integers, strict floating point, byte and String literals, numeric conversions, untyped `Ptr` memory access, external opaque types, and the aggregate C host ABI. The [`implementation roadmap`](../docs/implementation/roadmap.md) records the current milestone state; [`m0.md`](../docs/implementation/m0.md) records the original vertical slice.
