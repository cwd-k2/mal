...
fn public_cli_reports_help_version_and_usage_status() {
    ...
    assert_eq!(help, mal_compiler::cli::HELP);
    assert!(output.stderr.is_empty());
    assert!(!help.contains("emit-c"));
    assert!(help.contains("Optimization mode [default: production]"));
    assert!(help.contains("build <source.mal>            Build an executable (requires -o)"));
    assert!(help.contains("emit header <source.mal>"));
