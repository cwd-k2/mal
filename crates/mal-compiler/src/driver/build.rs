...
/// Returns the single C++ source that carries the linked program as assembly.
pub fn emit_atcoder(source_path: &Path, options: &ToolchainOptions<'_>) -> Result<String, Error> {
    let generated = generate_build_inputs(
        source_path,
        options.artifact_directory,
        options.optimization,
    ...
    Ok(render_atcoder_submission(&generated.graph, &assembly))
}
