...
| `backend/llvm` | admission済みexecution planをtarget tripleとdata layoutを持つLLVM moduleおよびC shimへ変換。現在はcapture-freeなfirst-class function、managed productとsum、direct・indirect call、self-tail edge、managed direct-self continuation frame、transportableなextern callをadmit |
