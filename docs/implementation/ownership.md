...
zeroにする。borrowされたoperandの最後のuseがそのbindingにある場合、operandのlenderも同じ時点でdeadになるため、operandと一緒に
releaseする。これはowner responsibilityの終了であり、optimization設定によらない。`backend/llvm/optimization/symbol_concat`が有効で、
