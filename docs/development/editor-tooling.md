...
invalidateしてdiagnosticを再生成する。diagnostic生成に成功した同一document versionのfrontend analysisを再利用し、
semantic indexは最初のsemantic requestで構築する。incremental analysisとinvalid sourceからの部分的semantic resultは
対象外とする。
