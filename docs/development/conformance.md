...
| [`types`: Engram](../spec/types.md#engram) | P/N: `checks_engram_literals_as_immutable_bytes`、`rejects_unsupported_or_mistyped_engram_operations`（`compiler/tests/check.rs`） | `emits_static_engram_bytes_that_survive_closure_escape`（`compiler/tests/c_emit.rs`） |
| [`memory`: Ptr、storage幅、primitive](../spec/memory.md) | P/N/E: `resolves_memory_primitives_and_the_ptr_type`、`resolves_the_type_in_a_storage_size_expression`（`compiler/tests/resolve.rs`）、`checks_ptr_extern_signatures_and_memory_primitives`、`checks_memory_primitives_for_every_supported_value_type`、`gives_every_memory_function_a_first_class_function_type`、`checks_storage_sizes_for_scalar_ptr_and_engram_types`、`rejects_storage_sizes_without_a_memory_representation`、`rejects_mistyped_memory_operations`（`compiler/tests/check.rs`） | `executes_unaligned_ptr_access_for_every_numeric_scalar`、`executes_unaligned_ptr_value_access`、`executes_unaligned_engram_descriptor_access`、`executes_first_class_memory_functions_through_closure_calls`、`executes_target_storage_size_expressions`（`compiler/tests/c_emit.rs`）、`pointer-tree` example（`compiler/tests/driver.rs`） |
| [`types`: Unit](../spec/types.md#unit) | P/E: `checks_function_application_and_zero_argument_unit_lowering`（`compiler/tests/check.rs`） | `emits_the_basic_host_abi_and_executes_the_host_example`（`compiler/tests/c_emit.rs`） |
...
| [`expressions`: case](../spec/expressions.md#case) | P/N/E: `parses_sum_injection_and_case_arms`、`rejects_the_old_case_syntax`（`compiler/tests/parser.rs`）、`case_pattern_and_block_bindings_share_an_arm_local_scope`（`compiler/tests/resolve.rs`）、`checks_case_exhaustiveness_uniqueness_and_result_types`（`compiler/tests/check.rs`）、`keeps_case_arm_effects_inside_the_selected_arm`（`compiler/tests/anf.rs`） | `executes_sum_injection_and_case`（`compiler/tests/c_emit.rs`） |
...
| [`execution`: trap](../spec/execution.md#trap) | trapを生じる各規則のfocused test | shift、division/remainder、Engram byte access、allocation、float conversion trap（`compiler/tests/c_emit.rs`） |
...
| 規範 | P / N / E | X |
|---|---|---|
| [`engrams`: 値とstorage](../spec/engrams.md#値とstorage) | P/E: `checks_engram_literals_as_immutable_bytes`（`compiler/tests/check.rs`） | static/captured/copy tests（`compiler/tests/c_emit.rs`） |
| [`engrams`: literal](../spec/engrams.md#literal) | P/N/E: Engram literal lexer tests（`compiler/tests/lexer.rs`） | `emits_static_engram_bytes_that_survive_closure_escape`（`compiler/tests/c_emit.rs`） |
| [`engrams`: operator](../spec/engrams.md#operator) | P/N/E: `parses_engram_length_and_byte_access_with_access_precedence`、`rejects_chained_engram_byte_access`（`compiler/tests/parser.rs`）、`checks_engram_operators_and_byte_wise_equality`、`rejects_unsupported_or_mistyped_engram_operations`（`compiler/tests/check.rs`） | `executes_engram_operators_and_byte_wise_equality`、`concatenates_engrams_as_immutable_bytes`、Engram access/concatenation trap tests（`compiler/tests/c_emit.rs`） |
| [`engrams`: mutable bytesとの分離](../spec/engrams.md#mutable-bytesとの分離) | P/N: opaque typeとunsupported operation tests（`compiler/tests/check.rs`） | `engram-round-trip` example（`compiler/tests/driver.rs`） |
...
| [`extern`: Engram lifetime](../spec/extern.md#engramのlifetime) | P/E: Engram型・signature検査（`compiler/tests/check.rs`） | copy、host mutation、allocation/length failure tests（`compiler/tests/c_emit.rs`） |
...
| [`c-host-abi`: type mapping](../spec/c-host-abi.md#type-mapping) | P/E: scalar/aggregate/opaque/Engram header tests（`compiler/tests/c_emit.rs`） | 各ABI round-trip test（`compiler/tests/c_emit.rs`） |
...
| [`grammar`: 文法概要](../spec/grammar.md#文法概要) | P/N/E: parser suite（`compiler/tests/parser.rs`）、`rejects_the_removed_case_arrow`（`compiler/tests/lexer.rs`） | 全checked-in example（`compiler/tests/driver.rs`） |
