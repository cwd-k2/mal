...
| [`memory`: Ptrとprimitive](../spec/memory.md) | P/N/E: `resolves_memory_primitives_and_the_ptr_type`（`compiler/tests/resolve.rs`）、`checks_ptr_extern_signatures_and_memory_primitives`、`checks_memory_primitives_for_every_supported_value_type`、`rejects_mistyped_or_first_class_memory_primitives`（`compiler/tests/check.rs`） | `executes_unaligned_ptr_access_for_every_numeric_scalar`、`executes_unaligned_ptr_value_access`（`compiler/tests/c_emit.rs`）、`pointer-tree` example（`compiler/tests/driver.rs`） |
