...
| [`types`: 直和](../spec/types.md#直和) | P/N/E: `checks_postfix_application_and_sum_continuations`、`rejects_invalid_sum_continuations_and_constructors`（`compiler/tests/check.rs`）、`rejects_single_member_sums_and_trailing_commas`（`compiler/tests/parser.rs`） | `branches_over_bool_and_unmanaged_sums_through_llvm`、`runs_first_class_sum_constructors_and_postfix_application_through_llvm`（`compiler/tests/driver/artifacts.rs`） |
| [`control`: return binder](../spec/control.md#return-binder) | P/N/E: `parses_return_binders_when_and_empty_forms`（`compiler/tests/parser.rs`）、`keeps_return_binders_local_to_their_invocation`（`compiler/tests/resolve.rs`）、`checks_explicit_return_binders_and_completion`、`rejects_return_binder_arity_and_value_use`（`compiler/tests/check.rs`） | `executes_explicit_and_sum_returns_through_the_existing_calling_convention`（`compiler/tests/driver/artifacts.rs`）、single return、sum全体return、2・3 variant returnを実行するchecked-in example（`compiler/tests/driver/examples.rs`）、indexed sum constructorをexample corpusから排除する`examples_construct_sum_variants_through_return_binders`（`compiler/tests/example_sources.rs`） |
...
| [`expressions`: 直和の除去](../spec/expressions.md#直和の除去) | P/N/E: `parses_postfix_and_unit_continuation_applications`（`compiler/tests/parser.rs`）、`checks_postfix_application_and_sum_continuations`、`rejects_invalid_sum_continuations_and_constructors`（`compiler/tests/check.rs`） | `branches_over_bool_and_unmanaged_sums_through_llvm`、`runs_first_class_sum_constructors_and_postfix_application_through_llvm`（`compiler/tests/driver/artifacts.rs`） |
