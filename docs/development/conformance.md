...
| [`memory`: Ptr、storage幅、primitive](../spec/memory.md) | P/N/E: `resolves_memory_primitives_and_the_ptr_type`、`resolves_the_type_in_a_storage_size_expression`（`compiler/tests/resolve.rs`）、`checks_ptr_extern_signatures_and_memory_primitives`、`checks_memory_primitives_for_every_supported_value_type`、`checks_storage_sizes_for_scalar_ptr_and_engram_types`、`rejects_storage_sizes_without_a_memory_representation`、`rejects_mistyped_or_first_class_memory_primitives`（`compiler/tests/check.rs`） | `executes_unaligned_ptr_access_for_every_numeric_scalar`、`executes_unaligned_ptr_value_access`、`executes_unaligned_engram_descriptor_access`、`executes_target_storage_size_expressions`（`compiler/tests/c_emit.rs`）、`pointer-tree` example（`compiler/tests/driver.rs`） |
