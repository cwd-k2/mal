...
| [`expressions`: application](../spec/expressions.md#application) | P/E: `checks_function_application_and_zero_argument_unit_lowering`、`checks_postfix_application_and_sum_continuations`（`compiler/tests/check.rs`）、`evaluates_an_argument_before_its_callee_and_application`（`compiler/tests/anf.rs`） | `runs_first_class_sum_constructors_and_postfix_application_through_llvm`、`runs_continuations_when_value_and_answer_function_types_overlap`（`compiler/tests/driver/artifacts.rs`） |
