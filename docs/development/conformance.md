...
| [`memory`: Ptrとprimitive](../spec/memory.md) | P/N/E: `resolves_memory_primitives_and_the_ptr_type`（`compiler/tests/resolve.rs`）、`checks_ptr_extern_signatures_and_memory_primitives`、`rejects_mistyped_or_first_class_memory_primitives`（`compiler/tests/check.rs`） | `executes_unaligned_ptr_scalar_access_through_the_host_abi`（`compiler/tests/c_emit.rs`） |
