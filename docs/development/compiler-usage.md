...
direct-self continuation frame、scalar・`Ptr`・managed productのextern callと数値scalar・`Ptr`のmemory primitiveは、pinned `clang`から取得したtarget tripleとdata layoutを
...
sumのexternは移行中のC body oracleへfallbackする。
