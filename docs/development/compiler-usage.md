...
LLVM backendが現在admitするcaptureを持たないunmanaged valueと単独の`Symbol`、direct call、self-tail edge、unmanagedな
...
managed aggregate、managed continuation frame、`Symbol` externは移行中のC body oracleへfallbackする。
