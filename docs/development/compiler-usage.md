...
direct-self continuation frame、scalar・`Ptr`・単独の`Symbol`のextern callと数値scalar・`Ptr`のmemory primitiveは、pinned `clang`から取得したtarget tripleとdata layoutを
...
managed aggregateのexternは移行中のC body oracleへfallbackする。
