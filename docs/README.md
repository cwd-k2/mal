...
| 直和除去のcontinuationからresult binderを参照する案を検討する | [直和除去continuationからのresult binder参照の導入計画](proposals/sum-continuation-result-binder.md) | [理論的根拠](proposals/redex-and-result-binder.md)、[直和の除去](spec/expressions.md#直和の除去)、[result boundaryとcompletion](spec/control.md)、[D049](history/decisions/D049.md) |
