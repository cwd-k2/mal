...
static inline MalOpaque_Mem mal_Mem_from_bits(uintptr_t bits) {
    return (MalOpaque_Mem){ .bits = bits };
}

static inline uintptr_t mal_Mem_bits(MalOpaque_Mem value) {
    return value.bits;
}

#define MAL_TAG_Response_0 UINT32_C(0)
#define MAL_TAG_Response_1 UINT32_C(1)
static inline uint32_t mal_tag_Response(MalType_Response value) {
    return value.tag;
}

static inline uint8_t mal_is_Response_0(MalType_Response value) {
    return value.tag == MAL_TAG_Response_0;
}

static inline MalType_Response mal_make_Response_0(
    void
) {
    return (MalType_Response){
        .tag = MAL_TAG_Response_0,
        .payload.variant_0 = { .unused = UINT8_C(0) },
    };
}

static inline uint8_t mal_is_Response_1(MalType_Response value) {
    return value.tag == MAL_TAG_Response_1;
}

static inline MalType_Response mal_make_Response_1(
    MalOpaque_Mem value_0,
    uint64_t value_1
) {
    return (MalType_Response){
        .tag = MAL_TAG_Response_1,
        .payload.variant_1 = {
            .field_0 = value_0,
            .field_1 = value_1,
        },
...
static inline MalOpaque_Mem mal_get_Response_1_0(
    MalContext *context,
    MalType_Response value
) {
    if (!mal_is_Response_1(value))
        mal_trap(context, "expected Response variant 1");
    return value.payload.variant_1.field_0;
}

static inline uint64_t mal_get_Response_1_1(
    MalContext *context,
    MalType_Response value
) {
    if (!mal_is_Response_1(value))
