...
layers likewise destroy every completed subtree before propagating `NodeResult[1]`.
...
not enforced by mal's types: `Allocator` and `Ptr` remain copyable, and the `[0]`/`[1]` convention and
ownership transfer must be followed manually.
